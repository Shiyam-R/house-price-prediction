...
- **`GET /version`** — reports exactly which model version, metrics, and API version are live
- **`GET /drift`** — lightweight drift monitoring: flags numeric input features whose recent traffic has shifted away from the training baseline
...
│       ├── baseline_stats.json # Training-data feature statistics, used by /drift
│       └── MODEL_CARD.md      # Human-readable documentation for this model version
├── scripts/
│   └── compute_baseline_stats.py  # Generates baseline_stats.json from raw training data
...
`GET /drift` compares a rolling window of recent `/predict` inputs against this model version's training-data baseline (`artifacts/<version>/baseline_stats.json`), flagging any numeric feature whose live mean has moved more than 2 standard deviations from its training-time mean.

```json
{
  "window_size_configured": 500,
  "features_drifting": 0,
  "total_features_tracked": 14,
  "features": {
    "OverallQual": {
      "sample_size": 118,
      "baseline_mean": 6.0993,
      "current_mean": 6.24,
      "z_score": 0.101,
      "drifting": false
    }
...
This is a lightweight, honest signal — in-memory, single-process, numeric features only — not a replacement for a dedicated drift-monitoring system in a real multi-instance deployment. See `app/drift_monitor.py` for the full scope and limitations, and `scripts/compute_baseline_stats.py` for how each version's baseline is generated from raw training data.
